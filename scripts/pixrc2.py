...
events = (uinput.BTN_JOYSTICK, uinput.ABS_X + (0, 255, 0, 0), uinput.ABS_Y + (0, 255, 0, 0),
          ...
          #uinput.KEY_BRIGHTNESSUP, uinput.KEY_BRIGHTNESSDOWN,
          #uinput.KEY_R, uinput.KEY_P, uinput.KEY_T, uinput.KEY_ESC,
          #uinput.KEY_SPACE)
...
def remap(indata):
    newdata = float(indata-1500)/1000.0
    return int(newdata*255.0+255.0/2.0)
...
def rcUnpack(packedData):
    ...
    if channels is not None and opts.show_rc:
        print channels #,dir(uinput)

    device.emit(uinput.ABS_RX, remap(int(channels[0])), syn=False) #throttle
    device.emit(uinput.ABS_RY, remap(int(channels[3]))) #yaw
    device.emit(uinput.ABS_X, remap(int(channels[1])), syn=False) #roll
    device.emit(uinput.ABS_Y, remap(int(channels[2]))) #pitc
    print remap(int(channels[0])), remap(int(channels[1])), remap(int(channels[2])), remap(int(channels[3]))
    '''
    if bh.changed():
        #print bh.button
        #print ""
        #print bh.buttonState['A']
        if bh.buttonState['A'][0] != "Released":
            device.emit(uinput.KEY_BRIGHTNESSUP, 1)
            print "A Pressed"
        if bh.buttonState['A'][0] == "Released":
            device.emit(uinput.KEY_BRIGHTNESSUP, 0)
            print "A Released"
        if bh.buttonState['B'][0] != "Released":
            device.emit(uinput.KEY_BRIGHTNESSDOWN, 1)
            print "B Pressed"
        if bh.buttonState['B'][0] == "Released":
            device.emit(uinput.KEY_BRIGHTNESSDOWN, 0)
            print "B Released"
        if bh.buttonState['RTL'][0] != "Released":
            device.emit(uinput.KEY_R, 1)
            print "RTL Pressed"
        if bh.buttonState['RTL'][0] == "Released":
            device.emit(uinput.KEY_R, 0)
            print "RTL Released"
        if bh.buttonState['Loiter'][0] != "Released":
            device.emit(uinput.KEY_P, 1)
            print "Loiter Pressed"
        if bh.buttonState['Loiter'][0] == "Released":
            device.emit(uinput.KEY_P, 0)
            print "Loiter Released"
        if bh.buttonState['Preset1'][0] != "Released":
            device.emit(uinput.BTN_JOYSTICK, 1)
            print "Preset1 Pressed"
        if bh.buttonState['Preset1'][0] == "Released":
            device.emit(uinput.BTN_JOYSTICK, 0)
            print "Preset1 Released"
        if bh.buttonState['Fly'][0] != "Released":
            device.emit(uinput.KEY_T, 1)
            print "Fly Pressed"
        if bh.buttonState['Fly'][0] == "Released":
            device.emit(uinput.KEY_T, 0)
            print "Fly Released"
        if bh.buttonState['CameraClick'][0] != "Released":
            device.emit(uinput.KEY_ESC, 1)
            print "CameraClick Pressed"
        if bh.buttonState['CameraClick'][0] == "Released":
            device.emit(uinput.KEY_ESC, 0)
            print "CameraClick Released"
        if bh.buttonState['Preset2'][0] != "Released":
            device.emit(uinput.KEY_SPACE, 1)
            print "Preset2 Pressed"
        if bh.buttonState['Preset2'][0] == "Released":
            device.emit(uinput.KEY_SPACE, 0)
            print "Preset2 Released"
    '''
...
def dsmSend(devName, baudRate):
    ...
    while True:
        ...
        if dsmBytes is not None and opts.show:
            print dsmBytes
...
parser.add_option("--show", action="store_true", default=False,
                  help="show DSM output")
parser.add_option("--show_rc", action="store_true", default=False,
                  help="show Unpacked RC output")
